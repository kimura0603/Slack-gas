...
function run() {
  ...
  channels.forEach(function (name, id) {
    ...
    // console.log("before call func retrieveMessages");
    retrieveMessages(id, name, nMessages);
    console.log(
      nMessages -
        nMessagesPre +
        " messages were retrieved from channel: " +
        name
    );
...
function paginate(method, payload) {
  // console.log('called func paginate');
  let results = [];
...
function getMessageData(messages, timesEdited, channelName) {
  ...
  messages.forEach(function (message) {
    ...
    if (message.type != "message") {
      console.log(
        "Non 'message' type message in channel: ",
        name,
        "\n",
        message
      );
      ...
      if (
        timesEdited.find(
          (e) => message.ts == e[0] && (editedTs === null || editedTs == e[1])
        )
      )
        return;
        ...
        message.files.forEach(function (file) {
          ...
          if (!("url_private_download" in file)) {
            console.warn(
              "In the channel: " +
                channelName +
                ", the following message has non-tombstone file w/o url_private_download"
            );
...
function fillMessages(messages, timesEdited, channelName, sheet) {
  // console.log('call fillMessages func');
...
function retrieveMessages(id, name) {
  // console.log("called func retrieveMessages");
  ...
  //1.スレッドのメッセージ一覧を集計。各メッセージの返信は後半部で取得
  let oldest = "0";
  if (!FULL_CHECK) {
  ...
  // console.log('after fillMessages for timeline');
  // console.log("threadTs");
  // console.log(threadTs);
  // console.log("debug1");
  if (nMessages >= MAX_MESSAGES) return nMessages;

  const threadTsFilled = sheet
    .getRange("D:D")
    .getValues()
    .filter(function (ts) {
      return ts[0] === "";
    });
  if (threadTsFilled.length != 0) {
    fillThreadTsForMessageWithoutReply(sheet);
  }

  //bugっぽい？ CHECK_THREAD_TS_IN_SHEET = trueにするとreplyが重複して挿入されてしまう。
  if (CHECK_THREAD_TS_IN_SHEET) {
    let threadTsInSheet = sheet
      .getRange("D:D")
      .getValues()
      .filter(function (ts) {
        return ts != "";
      });
    // console.log("threadTsInSheet before shift()");
    // console.log(threadTsInSheet);
    threadTsInSheet.shift();
    // console.log("threadTsInSheet after shift()");
    // console.log(threadTsInSheet);
    threadTs = [...new Set(threadTs.concat(threadTsInSheet))].map(function (
      ts
    ) {
      return String(ts);
    });
    // console.log("threadTs in CHECK_THREAD_TS_IN_SHEET");
    // console.log(threadTs);
    oldest = null;
  ...
  //2.各メッセージの返信を取得
  console.log("Retreiving Replies");
  ...
  //threadTsごとにソートすると、メッセージの直下にそれに紐づく返信が並ぶ形になる。
  sortAllInSheet(sheet, 4);
}
